...
G = ler_nos_csv('ApenasParadas/grafo_paradas_somente_PARADAS.csv')
G = ler_arestas_csv('ApenasParadas/grafo_paradas_somente_paradas_ARESTAS_COM_PESO.csv', G)
...
cmap = plt.get_cmap('cool')  # Mapa de cores
norm = plt.Normalize(vmin=0, vmax=max(pesos.values(), default=1))  # Normalização dos valores
...
plt.savefig('ApenasParadas/grafo_paradas.png', format='png', bbox_inches='tight')
plt.show()
